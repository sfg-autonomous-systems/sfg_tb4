...
{
    TypeSupport::TypeSupport(const rosidl_message_type_support_t *type_supports) : rmw_fastrtps_shared_cpp::TypeSupport(type_supports)
    {
    ...
    bool TypeSupport::serializeROSmessage(const void *ros_message, eprosima::fastcdr::Cdr &ser, const void *impl) const
    {
        assert(ros_message);
        assert(impl);
    ...
    bool TypeSupport::deserializeROSmessage(eprosima::fastcdr::Cdr &deser, void *ros_message, const void *impl) const
    {
        assert(ros_message);
        assert(impl);
        ...
        {
            RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("Fast CDR exception deserializing message of type %s.", get_name().c_str());
            return false;
        }
        catch (const std::bad_alloc &)
        {
            RMW_SET_ERROR_MSG_WITH_FORMAT_STRING("'Bad alloc' exception deserializing message of type %s.", get_name().c_str());
            return false;
        }
        ...
        const void *impl) const
    {
        ...
        if (key_is_unbounded_)
        {
            key_max_serialized_size_ = (std::max)(key_max_serialized_size_, key_callbacks_->get_serialized_size_key(ros_message));
            key_buffer_.resize(key_max_serialized_size_);
        }

        eprosima::fastcdr::FastBuffer fast_buffer(reinterpret_cast<char *>(key_buffer_.data()), key_max_serialized_size_);
        eprosima::fastcdr::Cdr ser(fast_buffer, eprosima::fastcdr::Cdr::DEFAULT_ENDIAN, eprosima::fastcdr::CdrVersion::XCDRv1);
